...
def read_running_port(directory=None, filename=LOCK_FILENAME):
    ...
    без да иска самия катинар — четенето на съдържание е позволено и на
    процеса, който НЕ държи заключването (то е само върху байт 0/EX режим,
    не пречи на четенето от друг процес).
