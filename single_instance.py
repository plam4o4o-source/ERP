...
def _default_dir():
    ...
    Одит (03.09.2026, находка №25): папката вече е МАШИННА, не
    потребителска. Докстрингът дотук твърдеше, че временната папка е
    „per-machine по дефиниция“ — това не следва от нищо:
    `tempfile.gettempdir()` чете `TMPDIR`/`TEMP`/`TMP`, а под Windows
    `%TEMP%` живее В ПОТРЕБИТЕЛСКИЯ ПРОФИЛ. Две едновременно активни
    сесии на един компютър (бърза смяна на потребители, RDP, „Изпълни като
    друг потребител“) дават две различни временни папки, значи две различни
    катинарни имена, значи и двата процеса минават проверката.

    Последицата не е просто „две копия“: `updater._machine_suffix()` хешира
    ИМЕТО НА КОМПЮТЪРА, тоест двата процеса ползват един и същ `<exe>.new`,
    един и същ bat и един и същ маркер за провал — буквално сценарият на
    находка №6 от 02.09, който `_machine_suffix` по конструкция не може да
    различи, защото машината е една.

    `%ProgramData%` е общ за всички потребители на компютъра и съществува
    на всяка поддържана Windows версия; при липса/невъзможност за запис се
    пада обратно към временната папка (по-добре per-user катинар, отколкото
    програма, която отказва да стартира). На POSIX `/tmp` вече е машинен.
