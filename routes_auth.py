...
def login():
    ...
    if request.method == "POST":
        ...
        if locked:
            ...
            # изтичаше след точно 5 минути, `is_locked_out` триеше записа и
            # броенето започваше от нула. Проверено с изпълнение (с вдигнати
            # per-IP и глобален праг, за да се изолира именно заключването по
            # акаунт): след 40 грешни опита оставаха 297 от 300 секунди —
            # тоест 35-те опита след петия не бяха добавили нищо, а
            # MAX_ATTEMPTS/LOCKOUT_SECONDS не ограничаваха нищо.
            #
...
def logout():
    ...
    # сесиите са подписани бисквитки, сървърът не пази нищо, значи всяко
    # снето копие оставаше напълно валидно до изтичането на
    # PERMANENT_SESSION_LIFETIME (12 часа). Проверено с изпълнение:
    # откраднатата бисквитка връщаше 200 СЛЕД изход.
    #
    # Механизмът вече съществува и се ползва при смяна на парола
    # (собствена и админска) — това е третият, най-често използваният
    # начин за прекратяване на сесия, който единствен не го викаше.
    # Приложението съзнателно не задава SESSION_COOKIE_SECURE (мрежов
    # режим по обикновен HTTP в офисния LAN), затова снемането на
    # бисквитка не е теоретично.
    #
    # Странична последица: изходът от едно устройство прекратява сесиите
    # на СЪЩИЯ потребител и другаде. За тази програма (един служител, един
    # компютър) това е желаното и по-безопасно поведение.
    ...
    if user_id is not None:
        try:
            ...
            con.execute("UPDATE users SET session_epoch = session_epoch + 1"
                        " WHERE id = ?", (user_id,))
            con.commit()
        except Exception:  # nosec B110 -- изходът НИКОГА не бива да се проваля:
            # ако базата е временно недостъпна, потребителят пак трябва да
            # може да излезе (бисквитката се трие във всички случаи по-долу).
            applog.log_exception("routes_auth.logout: неуспешно отнемане на сесията")
