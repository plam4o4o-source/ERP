...
def _parse_group_numbers(raw):
    ...
    Одит (03.09.2026, находка №8): вторият върнат елемент казва дали е
    ползвана РЕЗЕРВНАТА група. Падането към [1] е разумно (по-добре ред в
    карта, отколкото изгубен ред), но досега беше НЯМО — а всяко друго
    тихо влошаване в същия импорт се съобщава изрично (заглавен ред не на
    позиция 0, обединени клетки, орязване над 5000 реда, формули без
    стойности). Извикващият брои тези редове и предупреждава."""
    if raw is None:
        return [1], True
    parts = str(raw).split("+")
    nums = []
    for p in parts:
        ...
        if n not in nums:
            nums.append(n)
    return (nums, False) if nums else ([1], True)
...
def _parse_order_export(ws):
    ...
    for row in data_rows:
        ...
        row_groups, used_fallback = _parse_group_numbers(group_raw)
        if used_fallback:
            fallback_rows += 1
        for group in row_groups:
            groups.setdefault(group, []).append(item)
    # Одит (03.09.2026, находка №8): редовете с празна/нечетима групираща
    # клетка вече се броят и съобщават. Иначе палетна карта №1, която отива
    # при клиента и се лепи на палета, изброяваше стока, физически намираща
    # се другаде, и обявяваше завишен „Общ брой“ — при импорт от 5000 реда
    # операторът няма практическа възможност да го забележи на екрана.
    if fallback_rows:
