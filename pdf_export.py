...
def generate_document_pdf(title, number, barcode, fields, items, item_columns, totals_row=None):
    ...
    try:
        ...
        #
        # Обосновката „едно PDF ≈ 0.2 сек“ (находка №4 от 19.08) беше
        # измерена върху миниатюрен документ. Реално: 100 реда → 0.65 сек,
        # 300 реда → 2.1 сек. waitress работи с точно 8 нишки, а БЕЗ таван
        # осем едновременни износа на голяма фактура държат ВСИЧКИТЕ осем
        # нишки блокирани ~17 сек — през това време нито вход, нито табло,
        # нито запис на документ минава. По-добре ясна грешка на един
        # потребител, отколкото замразено приложение за целия офис.
