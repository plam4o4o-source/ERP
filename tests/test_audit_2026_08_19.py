...
def test_pdf_rendering_is_serialised_because_xhtml2pdf_shares_state():
    """Одит (19.08.2026, находка №4): `xhtml2pdf.files.TmpFiles` наследява
    `threading.local`, но държи списъка си като ClassVar с изменяема
    стойност по подразбиране — тоест списъкът е ОБЩ за всички нишки и
    `cleanFiles()` в края на всяко рендиране трие и чуждите временни
    файлове. Тестът документира и двете: че споделянето е реално (ако
    upstream го поправи, тестът ще падне и катинарът може да отпадне) и че
    ние сериализираме рендирането."""
    ...
    try:
        ...
        t2.start(); t2.join()
        assert "маркер-едно" in seen["две"], (
            "ако това падне, xhtml2pdf вече Е thread-safe и _render_lock "
            "може да бъде премахнат — но ПРОВЕРЕТЕ преди това")
    finally:
        pisa_files.files_tmp.files.clear()

    assert isinstance(pdf_export._render_lock, type(threading.Lock())), (
        "рендирането трябва да е сериализирано, докато споделянето по-горе е факт")
