...
def test_cloudflared_download_uses_a_unique_temp_name():
    ...
    # Одит (03.09.2026, находка №23): PID-ът сам по себе си НЕ различава
    # машини — а находката е формулирана именно като проблем между две
    # станции, споделящи една папка. В името трябва да влиза и отпечатък на
    # компютъра, както в updater._machine_suffix и app._startup_log_name.
    assert "_machine_suffix()" in src, (
        "находка №23: временното име трябва да носи и отпечатък на МАШИНАТА, "
        "не само PID (два компютъра лесно имат един и същ PID)")
