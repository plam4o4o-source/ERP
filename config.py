...
def validate_db_path(raw, allow_new=False):
    ...
    най-тежки, които тази програма може да има:

    * грешка в ИМЕТО на файла при съществуваща папка → следващият старт
      прави чисто нова празна база с admin/admin123, а истинските данни
      стоят недокоснати, но невидими (точно катастрофата, срещу която е
      писан коментарът при DEFAULTS по-горе — достижима през интерфейса);
    * грешка в ПАПКАТА → db.get_db() гърми, приложението пада в резервен
      режим и НЕ регистрира нито един маршрут, значи страницата
      „Настройки“, от която да се поправи, вече не съществува.

    Връща (грешка_или_None, нормализиран_път). Празна стойност е валидна
    (означава „базата до .exe-то“). `allow_new=True` е изричното
    потвърждение на потребителя, че наистина иска НОВА празна база.
    """
