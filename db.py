...
def init_db():
    # Одит (01.09.2026, девети одит, находка №11): цялото тяло е в
    # try/finally, за да се затвори връзката (и с нея — да се пусне
    # писателският катинар) при ВСЯКА грешка, не само при успех.
    #
    # Без него: `con.commit()` накрая може да гръмне със SQLITE_BUSY, когато
    # друг компютър държи катинар върху мрежовата база — точно сценарият, за
    # който `_create_app_or_explain` съществува. Тогава транзакцията остава
    # активна с ВЗЕТ RESERVED катинар (INSERT OR IGNORE/save_settings вече са
    # писали), а изключението се пази в closure-а на `_db_unavailable`
    # (app.py) до края на процеса — значи и traceback-ът, и кадърът на
    # init_db, и обектът `con` остават живи, ref-count никога не стига до
    # нула и катинарът НЕ се пуска. Резултат: собственият процес блокира и
    # трите си повторни опита, И всеки друг компютър в мрежата, до ръчно
    # спиране — а потребителят на --windowed .exe вижда само страницата
    # „базата е недостъпна“, без представа че сам я държи.
    #
    ...
    try:
        ...
        try:
            con.close()   # rollback на всяка недовършена транзакция
        except Exception:  # nosec B110 -- затварянето не бива да маскира
            pass           # истинската грешка, която ни е довела дотук


def _init_db_locked(con):
    """Тялото на init_db() — отделено, за да е гарантирано затварянето на
    връзката (виж try/finally там)."""
    ...
    con.commit()
    # Затварянето вече е в `finally` на init_db (находка №11) — тук не се
    # прави, за да не се затваря два пъти по успешния път.
